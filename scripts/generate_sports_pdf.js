...
async function main() {
  ...
  const outputDir = path.join(rootDir, 'เอกสารและรายชื่อคณะสีแสด_ปี69', 'แยกฝ่าย', 'กีฬา', 'รายชื่อนักกีฬา_PDF');
  ...
  if (fs.existsSync(regPath)) {
    ...
    regs.forEach(r => {
      ...
      if (sport) {
        const cat = routeStudentToCategory(sport.categories, r);
        if (cat) {
    ...
    if (st.duty) {
      ...
      duties.forEach(d => {
        ...
        if (matchedSportName) {
          ...
          if (sport) {
            const cat = routeStudentToCategory(sport.categories, st);
  ...
  console.log('--- 2. เรนเดอร์ไฟล์ PDF แยกตามชนิดกีฬา 7 ไฟล์ ---');
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    body { font-family: 'Sarabun', 'TH Sarabun New', sans-serif; margin: 0; padding: 0; color: #212121; background: #ffffff; font-size: 13px; line-height: 1.35; }
    .header-card { background: linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%); border-left: 6px solid #e65100; border-radius: 8px; padding: 16px 20px; margin-bottom: 18px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 2px 4px rgba(230, 81, 0, 0.08); }
    .header-left h1 { margin: 0 0 4px 0; color: #bf360c; font-size: 22px; font-weight: 800; }
    .header-left .subtitle { color: #5d4037; font-size: 13px; font-weight: 500; }
    .header-right .total-badge { background: #e65100; color: #ffffff; padding: 8px 16px; border-radius: 20px; font-weight: 700; font-size: 14px; }
    .category-block { margin-bottom: 18px; page-break-inside: avoid; }
    .category-title { background: #fff8e1; border-left: 4px solid #ff9800; padding: 7px 12px; font-size: 14px; font-weight: 700; color: #e65100; margin-bottom: 8px; border-radius: 0 6px 6px 0; }
    .badge { font-size: 12px; color: #795548; font-weight: normal; }
    .roster-table { width: 100%; border-collapse: collapse; margin-bottom: 6px; font-size: 12px; border: 1px solid #d6d6d6; }
    .roster-table th { background: #e65100; color: #ffffff; font-weight: 700; padding: 7px 4px; border: 1px solid #bf360c; text-align: center; font-size: 12.5px; }
    .roster-table td { padding: 6px 6px; border: 1px solid #e0e0e0; vertical-align: middle; }
    ...
    .text-muted { color: #757575; font-size: 11px; }
  </style>
</head>
<body>
  ...
  ${tablesHtml}
    ...
    const tempHtmlPath = path.join(outputDir, `temp_${sport.key}.html`);
    const finalPdfPath = path.join(outputDir, `${sport.name}.pdf`);
    fs.writeFileSync(tempHtmlPath, htmlContent, 'utf-8');

    try {
      execSync(`${edgePath} --headless --disable-gpu --print-to-pdf="${finalPdfPath}" --no-pdf-header-footer "${tempHtmlPath}"`, {
        stdio: 'pipe'
      });
      console.log(`[OK] สร้าง PDF สำเร็จ: ${finalPdfPath} (${fs.statSync(finalPdfPath).size.toLocaleString()} bytes)`);
    } catch (err) {
  ...
  console.log('\n--- เสร็จสิ้นการสร้างไฟล์ PDF ทั้ง 7 ชนิดกีฬา ---');
}
